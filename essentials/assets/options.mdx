...
When using the Plutus API, you can include option assets in your portfolio submission. Each option asset allows for the following customizable properties:

## Properties

- **Ticker**: The unique identifier of the underlying asset for the option (e.g., `"SPY"` for the S&P 500 ETF).
- **Is_Long**: Specifies whether the option position is `"Long"` or `"Short"`.
- **Style**: Indicates the exercise style, either `"American"` or `"European"`.
- **Is_Call**: Specifies if the option is a `"Put"` or `"Call"`.
